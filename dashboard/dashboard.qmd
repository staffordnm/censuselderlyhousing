...
filtered_data <- housing |>
  group_by(county) |>
  summarise(
    white_agg = sum(person_weight[white == 1 & rent_burden == "Rent Burden" & age >= 55]),
    black_agg = sum(person_weight[black == 1 & rent_burden == "Rent Burden" & age >= 55])
  ) |>
...
filtered_data2 <- housing |>
  group_by(county) |>
  summarise(
    white_agg = sum(person_weight[white == 1 & owner_burden == "Owner Cost Burden" & age >= 55]),
    black_agg = sum(person_weight[black == 1 & owner_burden == "Owner Cost Burden" & age >= 55])
  ) |>
  pivot_longer(cols = c(white_agg, black_agg),
               names_to = "race",
...
dot_plot_own <- ggplot(filtered_data2, aes(x = county, y = agg_own_burden, color = race)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_color_manual(values = c("black_agg" = "#a3dee8", "white_agg" = "#49497f"),
                     labels = c("black_agg" = "Black Resident Mortgage Burden", "white_agg" = "White Resident Mortgage Burdened")) +
    scale_y_continuous(labels = comma)
...
stacked_bar_zoomed <- ggplot(
  reactive_stack(), 
  aes(x = county, y = n_pop, fill = age_group)) +
